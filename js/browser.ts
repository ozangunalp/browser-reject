...
export const browserTest = (userAgent: string): Browser => {
  const a = (/Opera|Navigator|Minefield|KHTML|Chrome|CriOS/.test(userAgent)
    ? m(userAgent, [
  ...
  return {
    ...
    os: {
      name: (/(win|mac|linux|sunos|solaris|iphone|ipad)/.exec(navigator.platform.toLowerCase()) || [u])[0].replace(
        'sunos',
        'solaris'
      ),
    },

...
export const _pageSize = function () {
    const xScroll = window.innerWidth && (window as any).scrollMaxX ?
        window.innerWidth + (window as any).scrollMaxX :
        (document.body.scrollWidth > document.body.offsetWidth ?
            document.body.scrollWidth : document.body.offsetWidth);

    const yScroll = window.innerHeight && (window as any).scrollMaxY ?
...
export const reject = function (options?: Options): boolean {
    ...
    let html = '<div id="jr_overlay"></div><div id="jr_wrap"><div id="jr_inner">' +
    ...
    element.querySelectorAll<HTMLElement>('#jr_inner li').forEach(value => {
        value.style.background = 'transparent url("' + opts.imagePath + 'background_browser.gif") ' +
            'no-repeat scroll left top'
    });

    element.querySelectorAll<HTMLElement>('#jr_inner li .jr_icon').forEach(value => {
        // Dynamically sets the icon background image
        value.style.background = 'transparent url(' + opts.imagePath + 'browser_' +
            (value.parentElement!.id.replace(/jr_/, '')) + '.gif) no-repeat scroll left top';
